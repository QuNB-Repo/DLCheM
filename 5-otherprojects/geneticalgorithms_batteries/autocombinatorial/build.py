...
class autocombinatorial():
    def __init__(self,initial_mol_filepath='',mol_string='',max_number_n=6,max_number_fg=6,max_number_db=6,double=False,molecule_name=''):
        ...
        if double == True:
            double = random.choice((True,False,False,False,False,False))
    ...
    def show(self):
        ...
        fgs_copy = self.fgs
        if len(self.fgs) > 0:
            fgs_copy = fgs_copy[fgs_copy[:,0].argsort()]
        ...
        for i in range(len(fgs_copy)):
            molecule_id = molecule_id + '--' + str(fgs_copy[i][0]) + ',' + str(fgs_copy[i][1])  

        molecule_id = molecule_id + '-L'+str(self.linker)

        self.molecule_id = molecule_id
        save_mol = open('molecules/'+molecule_id+'.mol',mode='w')
        save_mol.write(self.mol_string)
    
    def add_n(self):

        number_N_to_add = random.randint(0,self.max_number_n)
        ...
        if number_N_to_add > 0:
            if self.double == True:
                position_to_add = random.sample((1,2,3,4,5), number_N_to_add)
            else:
                position_to_add = random.sample((1,2,3,4,5,6), number_N_to_add)
            

            for i in range(len(position_to_add)):

                #check valency of position to add first, if full, do not add! 
                available = utils.check_valency(position_to_add[i],self.mol_string,self.double)
    ...
    def add_fg(self):
        ...
        number_fg = random.randint(1,self.max_number_fg)
        ...
        for i in range(number_fg):
            ...
            if self.double == True:
                ...
                position_to_add = random.choice((1,2,3,4,5,6))

            #choose a functional group from  buildingblock files
            random_fg = random.choice((1,2,3,4,5,6,7,8,9))
            random_fg = str(random_fg)
            fg_data_file = './buildingblocks/fgroup/%s.mol' %(random_fg)

            #check availability of position
            available = utils.check_valency(position_to_add,self.mol_string,self.double)

            #if available, then do the replacing operation,
            if available == True:

                #save position and identty of fg, in that order
                self.fgs.append([int(position_to_add),int(random_fg)])

                #build functional group
                self.mol_string, self.benchmark_atom_number, self.benchmark_bond_number, fg_index = utils.build_fg(self.mol_string,fg_data_file,position_to_add,self.benchmark_atom_number,self.benchmark_bond_number)

                self.save_fg_index.append(fg_index)

    def add_db(self):
        
        number_db = random.randint(0, self.max_number_db)

        self.db_positions = []
        
        for i in range(number_db):
            ...
            position_for_double = random.choice((1,2,3,4,5,6))
            ...
            available_position = utils.check_valency(position_for_double,self.mol_string,self.double)
            available_neighbor = utils.check_valency(neighbor2,self.mol_string,self.double)
    ...
    def linker(self):
        ...
        if self.double == True:
            ...
            linker_data_file = './buildingblocks/linker/%s.mol' %(random_linker)

            self.linker = random_linker
        else:
            random_linker = random.choice((1,2,3,5))
            random_linker = str(random_linker)
            linker_data_file = './buildingblocks/linker/%s.mol' %(random_linker)       
    ...
    def find_fg(self):
        ...
        for file in os.listdir('buildingblocks/fgroup/'):

            fg_lines = []
            for line in fileinput.FileInput('buildingblocks/fgroup/'+file,inplace=0):
    ...
    def remove(self):
        ...
        if len(self.fgs) > 0:
            for i in range(len(self.fgs)):
                ...
                filename = './buildingblocks/fgroup/' + str(self.fgs[i][1]) +'.mol'

                #count number of atoms in functional group from fgroup file in building blockes
                fg_numb_atoms = utils.count_fg_atoms(filename)
                ...
                final_string = utils.adjust(new_string,indices_to_remove[len(indices_to_remove)-1],len(indices_to_remove),self.number_atoms,self.number_bonds, shift1)

                save_mol = open('molecules/'+self.molecule_id+'REM'+str(i)+'.mol',mode='w')
