...
def find_leavingatoms(CENTER_STRUCTURES,CENTER_LABELS,MOLECULE_LABELS,SKIP_SYMMETRIC_ATOMS):
    '''
    ...
    '''
    do_centers_exist = []
    all_matched_centersidxs = []
    all_matched_centerslabels = []
    #Need to go through each center specified,
    #To ensure that the number of matched labels for that center 
    #Is exactly that speciefied for that center. 
    #Don't try to do this FOR ALL centers at ONCE
    for center_i in range(len(CENTER_STRUCTURES)):

        matched_centerlabels = list((Counter(CENTER_LABELS[center_i]) & Counter(MOLECULE_LABELS)).elements())
        matched_centeridxs = [i for i, x in enumerate(MOLECULE_LABELS) if x in matched_centerlabels]
        
        if len(matched_centeridxs) == CENTER_STRUCTURES[center_i][1]:
            do_centers_exist.append('yes')

            if SKIP_SYMMETRIC_ATOMS == True:
                matched_centeridxs = matched_centeridxs[::2]

            all_matched_centersidxs.append(matched_centeridxs)
            all_matched_centerslabels.append(matched_centerlabels)

    all_matched_centerslabels = [item for sublist in all_matched_centerslabels for item in (sublist if isinstance(sublist, list) else [sublist])]
    all_matched_centersidxs =  [item for sublist in all_matched_centersidxs for item in (sublist if isinstance(sublist, list) else [sublist])]


    return do_centers_exist, matched_centerlabels, matched_centeridxs
...
def find_bondorderchange_atoms(CENTER_STRUCTURES,CENTER_LABELS,MOLECULE_LABELS):
    ...
    return all_matched_centersidxs
