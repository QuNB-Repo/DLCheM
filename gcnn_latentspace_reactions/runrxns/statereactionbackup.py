...
class RemoveBuild_LGFG_SingleBuildSite():
    ...
    def __init__(self,DATASET_FILEPATH,LABELS_FILEPATH,DEPTH,RXN_CENTERS,LEAVING_ATOMS,ATTACH_GROUP,ATTACH_GROUP_BONDS,ATTACH_GROUP_NUMB_ATOMSBONDS,ADD_BOND_TO_RXNSITE,REMOVE_BOND_FROM_RXNSITE,MOLECULES_RANGE,AVAILABLE_PROPERTIES,QM9_BOOL,SKIP,REACT_FILEPATH,REMOVELG_FILEPATH,PROD_FILEPATH):
        '''
...
'''         
    ...
    #           #columns for removal
                ...
                does_only1rc_exist = []
                matched_labels = []
                #the soudce of the FUCKING PROBLEM IS HERE!!!!!!
                #the source of the problem is HERE!!!!!!!!!!
                for each_atom in range(len(self.atomlabels_rxncenters)):
                    matched_label = None
                    for label in self.atomlabels_rxncenters[each_atom]:  # Start from index 1
                        if label in envlabels_of_molecule.tolist():
                            if len(matched_labels) > self.RXN_CENTERS[each_atom][1]:  # Another match found, so not unique
                                matched_label = None
                                break
                            else:
                                matched_label = label
                    ...
                    #ALSO NEED TO CHECK, that the unique label (out of all possible labels) is also found only once in the molecule
                    if np.count_nonzero(envlabels_of_molecule[envlabels_of_molecule == matched_label]) == self.RXN_CENTERS[each_atom][1]:
                        does_only1rc_exist.append('yes')
                    else:
                        does_only1rc_exist.append('no')  

                #does the environment exits?
                does_lg_exist = []
                indices_fg_found = []
                if self.LEAVING_ATOMS is not None:
                    for each_atom in range(len(self.atomlabels_leavinggroup)):
                        matched_label = None
                        ...
                        for label in self.atomlabels_leavinggroup[each_atom]:  # Start from index 1
                            if label in envlabels_of_molecule.tolist():
                                if matched_label is not None:  # Another match found, so not unique
                                    matched_label = None
                                    does_lg_exist.append('no')
                                    break
                                else:
                                    matched_label = label
                        ...
                        if matched_label is not None:
                            ...
                            indices_matched_label = np.where(envlabels_of_molecule == matched_label)[0]
                            number_allowed_lgatoms_toremove = self.LEAVING_ATOMS[each_atom][1]
                
                            for each_idx in range(number_allowed_lgatoms_toremove):
                                indices_fg_found.append(indices_matched_label[each_idx])

                            does_lg_exist.append('yes')
                        else:
                            does_lg_exist.append('no')
                elif self.LEAVING_ATOMS == None:  
                    does_lg_exist = ['yes' for each_atom in range(len(self.atomlabels_leavinggroup))]

                
                if 'no' in does_lg_exist or 'no' in does_only1rc_exist:
                    pass
                else:
                    ...
                    for each_atom in range(len(self.atomlabels_rxncenters)):
                        ...
                        reactcenter_index = np.where(envlabels_of_molecule == matched_labels[each_atom])[0]
                        
                        if len(reactcenter_index) > 1:
                            reactcenter_indices.append(reactcenter_index)
                        elif len(reactcenter_index) == 1: 
                            reactcenter_indices.append(int(reactcenter_index[0]))
                    
                    self.all_reactcenterindices.append(reactcenter_indices)
                    #THIS HAS TO BE DONE HERE!!! 
                    #now you can find the build indices using the filtered labels
                    prodcenter_indices = []
                    ...
                    filtered_envlabels_of_molecule = np.delete(envlabels_of_molecule,indices_fg_found)
                    for each_atom in range(len(self.atomlabels_rxncenters)):
                        #THIS HAS TO BE DONE HERE!!! 
                        #now you can find the build indices using the filtered labels
                        prodcenter_index = np.where(filtered_envlabels_of_molecule == matched_labels[each_atom])[0]

                        if len(prodcenter_index) > 1:
                            prodcenter_indices.append(prodcenter_index)
                        elif len(prodcenter_index) == 1: 
                            prodcenter_indices.append(int(prodcenter_index[0]))

                
                    self.all_prodcenterindices.append(prodcenter_indices)

                    self.removelg_xyz_string = RemoveBuild_LGFG_SingleBuildSite.RemoveAtoms_LG(self,indices_fg_found)
                    ...
                    matched_labels_add = []
                    if self.REMOVE_BOND_FROM_RXNSITE is not None:
                        for each_atom in range(len(self.atomlabels_removebond)):
                            matched_label = None

                            #First check that any one and ONLY one of the possible labels for a leaving atom is found
                            #BUT you do not necessarily need to check that it is found only once in the molecule
                            for label in self.atomlabels_removebond[each_atom]:  
                                if label in envlabels_of_molecule.tolist():
                                    if len(matched_labels_rem) > self.REMOVE_BOND_FROM_RXNSITE[each_atom][1]:  # ENOUGH matched found, stop
                                        matched_label = None
                                        break
                                    else:
                                        matched_label = label
                                        matched_labels_rem.append(matched_label)

                            if matched_label is not None:
                                #For the unique label found out of all the possibilities of labels for the substructure at this depth
                                #find the indice in the molecule that match this label
                                index_matched_label = np.where(envlabels_of_molecule == matched_label)[0]

                                if len(index_matched_label) > 1:
                                    indices_removebond.append(index_matched_label)
                                elif len(index_matched_label) == 1: 
                                    indices_removebond.append(index_matched_label[0])
                            
                        #WILL THERE BE A PROBLEM HERE OF DELETING MORE THAN ONE SITE? 
                    if self.ADD_BOND_TO_RXNSITE is not None:
                        for each_atom in range(len(self.atomlabels_addbond)):
                            matched_label = None

                            #First check that any one and ONLY one of the possible labels for a leaving atom is found
                            #BUT you do not necessarily need to check that it is found only once in the molecule
                            for label in self.atomlabels_addbond[each_atom]:  # Start from index 1
                                if label in envlabels_of_molecule.tolist():
                                    if len(matched_labels_add) > self.ADD_BOND_TO_RXNSITE[each_atom][1]:  # Another match found, so not unique
                                        matched_label = None
                                        break
                                    else:
                                        matched_label = label
                                        matched_labels_add.append(matched_label)
                            ...
                            if matched_label is not None:
                                ...
                                if len(index_matched_label) > 1:
                                    indices_addbond.append(index_matched_label)
                                elif len(index_matched_label) == 1: 
                                    indices_addbond.append(index_matched_label[0])
                            

                    #need to remove/pop all indices found from the relevant          
                    self.prod_xyz_string = RemoveBuild_LGFG_SingleBuildSite.Build_FG(self,prodcenter_indices,numb_atoms-len(indices_fg_found),indices_addbond,indices_removebond)
