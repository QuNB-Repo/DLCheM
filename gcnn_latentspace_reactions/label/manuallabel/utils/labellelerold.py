...
class label():
    def __init__(self,qm9data,start,end,element):
        self.start = start
        self.end = end
...
def label_c(self):
    ...
    for idx in range(start,end):
        ...
        name_xyz = utils.write_xyz_from_db(props,idx,label_direc)
        name_mol = utils.xyz_to_mol(idx,label_direc,rdkit=False)
...
#                        label = 47
                ...
                if total == 1:
                    ...
                    if countN == 1:
                        label = 46
                print(total,countH,countC,countN,countO)
                if label == 999:
                    print('999')
                    error_message = 'this functional group is unknown, molecule_id = %s' %(idx)
                    raise ValueError(error_message)
    ...
    for idx in range(start,end):
        ...
        #find and store all locations of nitrogens for this molecule
            ...
            for j in range(len(connected_positions)):
                ...
                if total == 3:
                    if countO == 1 and countC == 1 and countN == 1:
                        label = 33  ##new
                    if countH == 2 and countN == 1:
                        label = 31 ##NEW
                    if countO == 1 and countC ==1 and countH ==1:
                        label = 30 ##NEW
                    if countO == 1 and countC == 2:
                        label = 29 ## NEWW
                    if countO == 2 and countC == 1:
                        label = 28  #NEWWW1!! and allowed 
